...
## Next (ranked)
0. **Matt: confirm the pixel in Events Manager → Test Events.** The one hop this session couldn't close (needs the Business login). Expect stray `localhost` events from 09-10 verification, plus a real `Lead` from the confirmed 09-15 Substack signup.
1. **Matt: two newsletter photos** — `public/images/newsletter/hero.jpg` (lit mini on black, landscape ~1600px) and `workbench.jpg` (desk, ~1200px). Both slots check at build time and fall back to flat `bg-ink`, so the page holds without them; the hero is currently a plain black band. **The mobile hero fade cannot be checked until `hero.jpg` exists.**
2. **Run the description pass — ungated now.** 269 videos still point at the old `/#newsletter` anchor, which now matters because `/newsletter/` is a real page. `node scripts/update-descriptions.cjs --run --max 190`, then the remainder next day. Backup is fresh (09-15, 273 videos) and `--verify-urls` was green. **Token re-authed 09-15, so it dies around 09-22** — do it inside that window or re-auth first. `--only <id,id>` exists for one-off videos.
3. **Matt: revoke the Mailgun API key and delete the three `MAILGUN_*` vars from Netlify env.** The handler is gone as of 09-15, so the key is a live credential for an unused service. Local `.env.bak-premailgun` can go once done.
